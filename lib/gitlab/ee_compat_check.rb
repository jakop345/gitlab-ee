...
module Gitlab
  ...
  class EeCompatCheck
    ...
    def generate_patch(branch, patch_path)
      ...
      step("Fetching latest master", %w[git fetch origin master])
      status = step("Finding merge base with master", %W[git merge-base FETCH_HEAD #{branch}])
      raise "#{branch} is too far behind master, please rebase it!" unless status.zero?
    ...
    def ee_branch_compat_check!
      step("Checking out origin/#{ee_branch}", %W[git checkout -b #{ee_branch} origin/#{ee_branch}])
      ...
      unless status.zero?
        failed_files = output.lines.reduce([]) do |memo, line|
          if line.start_with?('error: patch failed:')
            ...
            memo << file unless file =~ /CHANGELOG\.md:\d+/
          end
          memo
        end
